...
_동일하게 보이는 것이 아니라, 동등한 수준의 정보, 기능 제공이 우선_

<br/>

- 다양한 웹 브라우저에서 정상적으로 작동하는 웹페이지를 만드는 방법론 중 하나
- 브라우저 마다 렌더링 엔진이 다르므로 특정 속성이나 기능을 지원하지 않는 경우가 있다.
- 브라우저에 따라서 기능과 보여지는것이 달라지면 안되기때문에 브라우저가 다르더라도 동일한 화면, 서비스를 제공받을 수 있게 웹페이지 제작 기술을 얘기하는 것

  → 웹 기술의 표준을 정의하는 공식기관 W3C(World Wide Web Consortium)에서 채택된 웹표준 기술을 기본으로 제작
  <br/>

<b>\*브라우저 호환성 체크 참고 링크</b>
